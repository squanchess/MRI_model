...
# 压制 nibabel 的 pixdim[0] (qfac) 警告，这类文件数据本身没问题
...
QC_FAST_MODE = False      # True = 只抽样几层切片检查（快 10-50 倍）


def quality_check_volume(filepath, min_dim=None):
    """对单个 NIfTI 文件执行质量检查。

    两种模式（由全局变量 QC_FAST_MODE 控制）：
      - fast 模式：只读 header + 抽样 3-5 层切片，速度快 10-50 倍
      - full 模式：读取完整体素数据，检查更严格

    检查项目：
      - 文件是否可被 nibabel 正常加载
      - 数据是否为 3D（4D 会在调用方通过数据集规则提前处理）
      - 是否包含 NaN 或 Inf（fast 模式下为抽样检查）
      - 是否近常数（std <= QC_MIN_STD）（fast 模式下为抽样估计）
      - 每个空间维度是否 >= min_dim
      - spacing（体素间距）是否存在且在合理范围内

    参数：
        filepath: NIfTI 文件路径
        min_dim:  每个空间维度的最小尺寸（None 时用全局 QC_MIN_DIM）
    ...
    info["voxel_count"] = int(np.prod(spatial_shape))

    # --- spacing 检查（纯 header，极快） ---
    try:
    ...
    if QC_FAST_MODE:
        ...
        try:
            ...
            samples = []
            for idx in indices:
                # img.dataobj 支持切片索引，只从磁盘读一层
                slc = np.asarray(img.dataobj[:, :, idx], dtype=np.float32)
                samples.append(slc)

            data_sample = np.concatenate([s.ravel() for s in samples])

            if np.any(np.isnan(data_sample)):
                return False, info, "contains_nan (sampled)"
            if np.any(np.isinf(data_sample)):
                return False, info, "contains_inf (sampled)"

            std_val = float(np.std(data_sample))
            mean_val = float(np.mean(data_sample))
            info["mean_intensity"] = round(mean_val, 4)
            info["std_intensity"] = round(std_val, 4)

            if std_val <= QC_MIN_STD:
                return False, info, "near_constant: std={:.2e} (sampled)".format(std_val)

        except Exception as e:
            return False, info, "fast_qc_read_failed: {}".format(str(e)[:120])
    else:
        # === 完整模式：读取全部体素 ===
        try:
...
def main():
    global QC_MIN_DIM, QC_FAST_MODE
    ...
    parser.add_argument(
        "--fast_qc", action="store_true",
        help="Fast quality check: sample a few slices instead of reading "
             "the full volume. ~10-50x faster, suitable for large datasets.",
    )
